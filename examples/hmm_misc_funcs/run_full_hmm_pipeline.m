function [hmm, hmmfname, hmmoptions, settings_prepare, hmm_input_spm_files, hmm_input_epoched_spm_files] = run_full_hmm_pipeline(S)
...
try parcellations_dir=S.parcellations_dir; catch error('S.parcellations_dir needed'); end
...
try num_states=S.num_states; catch, num_states=8; end
...
for ss=1:length(prep_sessions_to_do)
    ...
    spm_files{ss}=[spmfilesdir preproc_name '_session' num2str(session)];
    epoched_spm_files{ss}=[spmfilesdir preproc_name '_session' num2str(session) '_epoched'];
end
...
if do_prepare
    prep_parcellated_data(settings_prepare);
end
...
if (any(~isfile))
    warning('Invalid enveloped_files');
end
...
hmm.parcellation.file = hmm.parcellation.S.parcellation;
