...
for subnum = 1:nsubs
    disp(['Computing for subject number ' num2str(subnum)]);
    Dp = spm_eeg_load(Ds{subnum});
    embed.tres=1/Dp.fsample;
            
    % returns data as num_nodes x num_embeddings x ntpts
    [datap, num_embeddings] = prepare_data( ...
        Dp, normalisation, logtrans, embed, roinets_protocol, innovations_mar_order);
